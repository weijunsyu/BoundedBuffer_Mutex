...
#define BSIZE 3 //max num elements in the buffer
typedef struct {
	char buf[BSIZE]; //circular buffer
	int occupied; //amount of elements in the buffer
	int nextin, nextout; //next index (in and out)
	pthread_mutex_t mutex; //mutex of the buffer
	pthread_cond_t more;
	pthread_cond_t less;
} buffer_t;
buffer_t buffer;
...
// program entry point
//
int main( int argc, char *argv[] ){ //argument is the time to sleep
...
void* producerFunction(void * parm){
	...
	for( i=0 ;; i++){
		...
		// acquire lock
		if(pthread_mutex_lock(&(buffer.mutex)) == 0) printf("producer has the lock. \n");

		// debug info
		if(buffer.occupied >= BSIZE) printf("producer waiting, full buffer ... \n");
		...
		// add to the buffer
		//take item[i] and set buffer.next to it
		buffer.buf[buffer.nextin++] = item[i];
		buffer.nextin %= BSIZE; //loop back if overflow
		buffer.occupied++;

		// debug info
		printf("producing object number: %i [%c]\n", i, item[i]);

		// signal the consumer, release the lock
		pthread_cond_signal(&(buffer.more)); //signal consumer
		pthread_mutex_unlock(&(buffer.mutex)); //release lock
...
void* consumerFunction(void * parm){
	...
	for( i=0 ;; i++ ){
		...
		pthread_cond_signal(&(buffer.less)); //signal producer
		pthread_mutex_unlock(&(buffer.mutex)); //release lock
